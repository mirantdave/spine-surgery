...
export function App() {
  ...
  const handleSaveProtocolCompliance = async (entry: PatientProtocolCompliance) => {
    ...
    try {
      ...
      // Fall through to local-only update when the API is offline.
    }
  ...
  const handleImportFromConnectorsToOtNote = async () => {
    ...
    const updatedNote: OperativeNote = {
      ...
      assistantSurgeon: existingNote?.assistantSurgeon || selectedPatient.fellowOrResident || 'Dr. Amritesh Singh, Junior Spine Consultant',
      anesthesiologist: con.pacAnesthesia?.recordedBy?.name 
        ...
        : (existingNote?.anesthesiologist || 'Dr. Kashyap Rameshchandra Shah (Head, Anaesthesia)'),
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPatients(prev => prev.map(p => 
          p.id === selectedPatient.id ? { ...p, departmentConnectors: saved } : p
        ));
      }
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Main Workspace Layout: Sidebar + Clinical Stage */}
      <div style={{ display: 'flex', flex: 1, height: 'calc(100vh - 58px)', overflow: 'hidden' }}>
        ...
        {selectedPatient ? (
          <main style={{
            ...
            {/* Active Patient Hero Banner with Embedded Live 2D Executed Surgery Viewport */}
            <div style={{
              padding: '18px 20px',
              display: 'grid',
              ...
              {/* Left Column: Patient Demographics, Clinical Details & Quick Actions */}
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12px', color: 'var(--text-secondary)', marginTop: '5px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '6px', flexWrap: 'wrap' }}>
                      <span style={{
                        ...
                        {selectedPatient.affectedLevels.join('-') || 'Spine'}
                      </span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', paddingTop: '8px', borderTop: '1px solid var(--border-color)' }}>
                  <button
                    ...
                    title="Open AI Sentinel & Clinical Risk Analysis"
                  >
                    <Sparkles size={11} color="#0071e3" />
                    <span>Smart Sentinel & Risk Scores</span>
                  </button>
                  <button
                    onClick={() => setPrintType('ot-note')}
                    style={{
                      fontSize: '11px',
                      padding: '4px 11px',
                      borderRadius: '999px',
                    ...
                    title="Print official operative record"
                  >
                    <Printer size={11} color="#0071e3" />
                    <span>Print OT Note</span>
                  </button>
                  <button
                    onClick={() => setPrintType('discharge-summary')}
                    style={{
                      fontSize: '11px',
                      padding: '4px 11px',
                      borderRadius: '999px',
                  ...
                  <button
                    onClick={() => setPrintType('prescriptions')}
                    style={{
                      fontSize: '11px',
                      padding: '4px 11px',
                      borderRadius: '999px',
                  ...
                  <button
                    ...
                    title="Expand 2D X-Ray animation to full screen modal"
                  >
                    <Maximize2 size={11} />
              ...
              {/* Right Column: Directly Visible Live 2D Executed Surgery X-Ray Animation */}
              <div style={{
                height: '175px',
                position: 'relative',
                borderRadius: '12px',
                ...
                <Surgery2DXRayViewer 
                  ...
                  height="100%"
                  variant="inline"
                  onExpand={() => setSurgeryXRayModalOpen(true)}
                />
              </div>
            </div>

            {/* Apple Minimalist Segmented Navigation Bar */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', margin: '4px 0' }}>
              <div className="apple-segmented-nav" style={{ overflowX: 'auto', maxWidth: '100%' }}>
                {[
                  { id: 'pathway', label: 'Care Pathway', icon: Route, count: 'Day-Wise' },
                  { id: 'reports', label: 'Clinical Reports', icon: FileBarChart, count: '6 Reports' },
                  { id: 'ot-note', label: 'Operative Record', icon: FileText, count: selectedPatient.operativeNote ? '✓' : '' },
                  { id: 'inpatient', label: 'Inpatient Care', icon: Stethoscope, count: (selectedPatient.wardRounds || []).length ? `${(selectedPatient.wardRounds || []).length}` : '' },
                  { id: 'discharge', label: 'Discharge Summary', icon: FileCheck2, count: selectedPatient.dischargeSummary ? '✓' : '' },
                  { id: 'protocols', label: 'Clinical Protocols', icon: BookOpenCheck, count: `${matchProtocolsToPatient(selectedPatient).length} Active` },
                  { id: 'connectors', label: 'Hospital Connectors', icon: Building2, count: selectedPatient.departmentConnectors ? '9 Feeds' : '' },
                  { id: 'anatomy', label: 'Spine Column', icon: Layers }
                ].map((tab) => {
                  const Icon = tab.icon;
            ...
            <div style={{ flex: 1 }}>
              ...
              {activeTab === 'inpatient' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {/* Clean Inpatient Sub-Navigation (Apple Segmented Bar) */}
                  <div style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: '2px',
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          <SubIcon size={13} color={isSubActive ? '#0071e3' : '#9ca3af'} />
                          <span>{sub.label}</span>
                  ...
                  <div className="glass-panel" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginTop: '10px' }}>
                      <div style={{ background: 'var(--bg-primary)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                        ...
                        <div style={{ fontSize: '16px', fontWeight: 800, color: '#38bdf8', textTransform: 'capitalize' }}>
                    ...
                    <div style={{ background: 'rgba(14, 165, 233, 0.05)', padding: '14px', borderRadius: '10px', border: '1px solid rgba(14, 165, 233, 0.2)', marginTop: '10px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                        <h4 style={{ fontSize: '13px', fontWeight: 700, color: '#0071e3', margin: 0 }}>
                          Active Spine Surgical Plan
                        </h4>
                        <button
                          onClick={() => setSurgeryXRayModalOpen(true)}
                          className="btn btn-secondary"
                          style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '999px', color: '#0071e3', fontWeight: 600 }}
                        >
                          ⛶ Fullscreen X-Ray View
                        </button>
                      </div>
                      <div style={{ fontSize: '12px', color: 'var(--text-primary)', marginBottom: '4px' }}>
                        <strong>Procedure: </strong>{selectedPatient.plannedProcedure}
                      </div>
                      <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                        <strong>Primary Indication: </strong>{selectedPatient.primaryDiagnosis}
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: '8px' }}>
                      <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <span>Executed Surgery 2D X-Ray Animation (Lateral & AP)</span>
                      </div>
                      <Surgery2DXRayViewer patient={selectedPatient} height="460px" variant="full" />
                    </div>
                  </div>
                </div>
              )}
